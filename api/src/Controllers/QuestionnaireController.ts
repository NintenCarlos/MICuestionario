  ...
  try {
    ...
    for (const question of body.questions) {
      if (
        !question.title ||
        !question.type ||
        typeof question.isMandatory == "undefined"
      ) {
    ...
    if (isInvalidQuestion) {
      res.status(400).json({
        msg: "Faltan datos para crear un cuestionario (en preguntas)",
      });
      return;
    }
    const createdQuestionnaire = await QuestionnairesModel.create(
      questionnaire
    );
    for (const question of body.questions) {
      const objQuestion = {
...
export const getQuestionnaires = async (
  ...
  try {
    ...
    res.status(200).json({
      msg: "Cuestionarios obtenidos con éxito",
      questionnaires,
    });
    return;
  } catch (error) {
